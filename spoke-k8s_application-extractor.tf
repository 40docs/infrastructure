data "azurerm_public_ip" "hub_nva_vip_extractor_public_ip" {
  count               = var.application_extractor ? 1 : 0
  name                = azurerm_public_ip.hub_nva_vip_extractor_public_ip[0].name
...
resource "azurerm_dns_cname_record" "extractor" {
  ...
  resource_group_name = azurerm_resource_group.azure_resource_group.name
  ttl                 = 300
  record              = data.azurerm_public_ip.hub_nva_vip_extractor_public_ip[0].fqdn
}

resource "azurerm_public_ip" "hub_nva_vip_extractor_public_ip" {
  count               = var.application_extractor ? 1 : 0
  name                = "hub_nva_vip_extractor_public_ip"
